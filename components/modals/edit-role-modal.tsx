 ...
 * 
 * Modal for editing existing roles with name, color, icon selection,
...
export function EditRoleModal() {
  ...
  const { space, serverId, userPowerLevel = 50, role } = data;

  const form = useForm<EditRoleForm>({
    resolver: zodResolver(editRoleSchema),
    defaultValues: {
      name: role?.name || "",
      color: role?.color || getDefaultColorForPowerLevel(25),
      icon: (role?.permissions ? "shield" : "shield") as RoleIcon,
      powerLevel: role?.powerLevel || 25,
      template: "member",
    },
  });
  ...
  useEffect(() => {
    if (role) {
      form.reset({
        name: role.name,
        color: role.color,
        icon: "shield", // Default icon since we don't have icon info in MatrixRole type
        powerLevel: role.powerLevel,
        template: "member",
      });
      setPermissions((role.permissions || getClosestPermissionTemplate(role.powerLevel)) as unknown as MeloPermissions);
    }
  }, [role, form]);
  ...
  const onSubmit = async (values: EditRoleForm) => {
    if (!serverId || !role?.id) {
      setError("Server ID and Role ID are required");
      return;
    }
    ...
    try {
      ...
      // Update the role
      await updateCustomRole(serverId, role.id, {
