...
import { Copy, Download, LinkIcon, QrCode, Plus, Trash2, Settings, ExternalLink } from "lucide-react";
...
export function InviteGenerator({ spaceId, spaceName }: InviteGeneratorProps) {
  ...
  const [existingInvites, setExistingInvites] = useState<InviteLink[]>([]);
  ...
  const loadExistingInvites = (service: MatrixInviteService) => {
    const invites = service.getInvites(spaceId);
  ...
  const handleCreateInvite = async () => {
    ...
    try {
      ...
      if (result.success && result.invite) {
        ...
        // Refresh the list
        loadExistingInvites(inviteService);
  ...
  const handleDeleteInvite = (invite: InviteLink) => {
    if (!inviteService) return;

    inviteService.deleteInvite(spaceId, invite.url);
    loadExistingInvites(inviteService);
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          {invite.alias && <Badge variant="secondary">Alias</Badge>}
                        </div>
