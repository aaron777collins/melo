# Melo User Guide

Welcome to Melo! This comprehensive guide will help you get started and make the most of your chat experience.

## 📚 Quick Navigation

### 🚀 [Getting Started](./getting-started/README.md)
- [Account Setup](./getting-started/account-setup.md)
- [Joining Your First Server](./getting-started/joining-server.md)
- [Basic Navigation](./getting-started/navigation.md)

### ✨ [Features](./features/README.md)
- [Servers & Channels](./features/servers-channels.md)
- [Messaging & Chat](./features/messaging.md)
- [Voice & Video Calls](./features/voice-video.md)
- [File Sharing](./features/file-sharing.md)
- [Member Management](./features/member-management.md)

### ⚙️ [Settings](./settings/README.md)
- [User Profile](./settings/user-profile.md)
- [Server Settings](./settings/server-settings.md)
- [Privacy & Security](./settings/privacy-security.md)
- [Appearance](./settings/appearance.md)

### 🛠️ [Troubleshooting](./troubleshooting/README.md)
- [Common Issues](./troubleshooting/common-issues.md)
- [Connection Problems](./troubleshooting/connection-problems.md)
- [Audio/Video Issues](./troubleshooting/audio-video-issues.md)
- [Mobile App Issues](./troubleshooting/mobile-issues.md)

---

## About Melo

Melo is a modern communication platform that brings people together through:

- **Servers**: Organized communities with multiple channels
- **Real-time Messaging**: Instant communication with emoji reactions and file sharing  
- **Voice & Video Calls**: High-quality audio and video communication
- **Mobile-First Design**: Seamless experience across all devices
- **Rich Customization**: Themes, roles, and server customization

## Need Help?

- 📧 **Support**: Contact our support team
- 💬 **Community**: Join our official support server
- 📱 **Mobile**: This guide is optimized for mobile viewing

---

*Last updated: February 22, 2026*