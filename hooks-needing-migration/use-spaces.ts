...
export function useUnreadDMCount(): number {
  ...
  useEffect(() => {
    const updateUnreadCount = async () => {
      ...
      try {
        const { getDMRooms } = await import("@/apps/web/services/matrix-dm");
        const dmRooms = await getDMRooms();
        
        let totalUnread = 0;
        dmRooms.forEach(room => {
          totalUnread += room.getUnreadNotificationCount() || 0;
        });
        
        setUnreadCount(totalUnread);
      } catch (error) {
        console.error("Failed to get DM unread count:", error);
        setUnreadCount(0);
      }
    };
    ...
    if (client) {
      const handleRoomUpdate = () => updateUnreadCount();
      client.on("Room.timeline", handleRoomUpdate);
      client.on("Room.receipt", handleRoomUpdate);

      return () => {
        client.off("Room.timeline", handleRoomUpdate);
        client.off("Room.receipt", handleRoomUpdate);
      };
    }
  }, [client]);
